  ...
  return (
    <>
          ...
          <div className="grid gap-5">
            {introStats.map((stat) => (
              <div key={stat.label} className="card-surface p-6 text-center">
                <p className="font-heading text-4xl text-saffron">{stat.value}</p>
                <p className="mt-2 text-sm uppercase tracking-[0.24em] text-stone-600">
                  {stat.label}
                </p>
              </div>
            ))}
          ...
          <div className="card-surface p-8 sm:p-10">
            ...
            <div className="mt-8 space-y-4">
              {timingData.map((item) => (
                <div
                  key={item.label}
                  className="flex items-center justify-between rounded-2xl bg-ivory px-5 py-4"
                >
      ...
      <section className="pb-20">
        <div className="section-shell">
          <div className="rounded-[2rem] bg-gradient-to-r from-saffron to-gold px-8 py-12 text-center text-white shadow-aura sm:px-12">
            ...
            <Link
              to="/donations"
              className="mt-8 inline-flex rounded-full bg-white px-6 py-3 text-sm font-semibold text-ink transition hover:bg-ink hover:text-white"
            >
              Make a Donation
            </Link>
          </div>
